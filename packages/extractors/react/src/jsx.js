...
const allUnknown = (reason) => {
    ...
    s.color.border = u();
    s.shape.radius = u();
    s.shape.borderWidth = u();
    s.shape.shadow = u();
    return s;
};
export const extractReact = (source, file) => {
            ...
            if (attr) {
                ...
                if (v?.type === 'StringLiteral') {
                    ...
                    if (e.type === 'StringLiteral')
                        style = resolveTailwindClasses(e.value);
                    else if (e.type === 'CallExpression')
                        style = allUnknown('unresolved-call');
                    else
                        style = allUnknown('dynamic-expression');
                }
            }
            const textChild = path.node.children.find((c) => c.type === 'JSXText' && c.value.trim().length > 0);
            const parentEl = path.findParent((p) => p.isJSXElement());
