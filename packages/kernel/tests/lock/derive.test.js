...
const writeTailwind = (d, body) => writeFile(join(d, 'tailwind.config.mjs'), body);
describe('deriveLock', () => {
    ...
    it('falls back to CSS custom properties when no tailwind config exists', async () => {
        ...
        await writeFile(join(dir, 'src/globals.css'), ':root { --space-1: 4px; --space-2: 8px; --color-primary: #1F4B3F; }');
        const { lock } = await deriveLock(dir);
...
describe('scanComponents', () => {
    it('registers components and their variant names', async () => {
        ...
        await writeFile(join(dir, 'src/ui/Button.tsx'), `const variants = { primary: '', ghost: '' }
