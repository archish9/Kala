...
const origin = { kind: 'class', raw: 'p-[13px]' };
...
describe('runRules — aggregate kind', () => {
    ...
    it('attaches a surface id when scope is surface', () => {
        const surfaceRule = { ...flatRule, scope: 'surface' };
...
describe('runRules — predicate rules', () => {
    ...
    const card = (id, parent, children = []) => makeNode({
        id, name: 'div', parent, children,
        style: { ...base, shape: { ...base.shape, radius: known({ px: 12 }, origin) } }
    });
    const doc = (nodes) => ({ file: 'a.tsx', framework: 'react', imports: [], dataSources: [], nodes });
    const predicates = {
        'nested-card': (node, ctx) => {
            const parent = ctx.doc.nodes.find((n) => n.id === node.parent);
            if (!parent || parent.style.shape.radius.state !== 'known')
                return null;
            return {
                rule: 'nested-card', sev: 'warn', file: ctx.doc.file,
    ...
    it('survives a predicate that throws', () => {
        ...
        const r = runRules([d], [cardRule], {}, boom);
        expect(r.findings).toEqual([]);
