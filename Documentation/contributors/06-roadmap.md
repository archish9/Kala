...
| — | **`npx kala install`** — a one-line installer that detects the harness, writes the MCP config block, and copies `skills/kala/SKILL.md` and the `/kala` command into place. Largely overtaken: the Claude Code plugin installs all of it in two commands, and every other client needs one JSON block naming `npx -y kala-mcp`, with no clone and no build. What remains uncovered is harness detection for the clients that have no plugin system. | Superseded for Claude Code; candidate elsewhere |
