...
> **You probably do not need this page.** Your agent calls these tools for you — see
> [What to say](03-prompts.md) for how to drive them in plain language. This reference is
> for people scripting kala directly or debugging what an agent sent.

All eight MCP tools: parameters, responses, worked examples, and failure behaviour.
...
The six surfaces and their aliases: [Surfaces and actions](07-surfaces-and-actions.md).
...
|---|---|---|---|
| `dir` | string | yes | Absolute path to the project root |
| `action` | enum | yes | One of the 13 actions |
| `target` | string | no | File or surface the action applies to |

The 13 actions, each with its intent and the token groups it draws on:
[Actions](07-surfaces-and-actions.md#actions-13). That table is generated from the pack
data, so it is the one place the list is maintained.
...
**Next:** [What kala checks](08-what-kala-checks.md) documents every rule `verify` runs.
